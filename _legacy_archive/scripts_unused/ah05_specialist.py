# scripts/ah05_specialist.py
"""
ESPECIALISTA AH 0.5
====================
Entrena patrones SOLO para handicap 0.5 y -0.5
Entiende que:
- AH +0.5 = Local es LIGERO favorito
- AH -0.5 = Visitante es LIGERO favorito

Busca señales específicas para este handicap tan común.
"""
...
def extract_ah05_features(m: Dict) -> Dict:
    """Features específicas para AH 0.5."""
    f = {}
    ...
    f['ah'] = ah
    f['ou'] = ou
    f['fav_local'] = ah > 0  # Local da 0.5
    f['fav_visita'] = ah < 0  # Visitante da 0.5
    ...
    try:
        ...
        f['h_bottom'] = hr >= 15
        f['a_bottom'] = ar >= 15
        f['h_mejor_rank'] = hr > 0 and ar > 0 and hr < ar
        f['a_mejor_rank'] = hr > 0 and ar > 0 and ar < hr
        f['ranks_cercanos'] = hr > 0 and ar > 0 and abs(hr - ar) <= 3
    except:
        pass
    ...
    try:
        ...
        f['h_medio'] = 0.35 <= f['h_wr'] < 0.6
        f['h_debil'] = f['h_wr'] < 0.35
    ...
    try:
        ...
        f['h_gf_avg'] = h_gf / h_pj
        f['h_gc_avg'] = h_gc / h_pj
        f['a_gf_avg'] = a_gf / a_pj
        f['a_gc_avg'] = a_gc / a_pj
        
        f['h_anota_mucho'] = f['h_gf_avg'] >= 1.5
        f['a_anota_mucho'] = f['a_gf_avg'] >= 1.5
        f['h_recibe_poco'] = f['h_gc_avg'] <= 0.8
        f['a_recibe_poco'] = f['a_gc_avg'] <= 0.8
        f['h_recibe_mucho'] = f['h_gc_avg'] >= 1.5
        f['a_recibe_mucho'] = f['a_gc_avg'] >= 1.5
    ...
    if sc:
        ...
        if 'Tiros a Puerta' in stats:
            diff = stats['Tiros a Puerta']['h'] - stats['Tiros a Puerta']['a']
            f['ph_tiros_dom'] = diff >= 4
            f['ph_tiros_dominado'] = diff <= -4
    ...
    if sc:
        ...
        if 'Tiros a Puerta' in stats:
            diff = stats['Tiros a Puerta']['a'] - stats['Tiros a Puerta']['h']
            f['pa_tiros_dom'] = diff >= 4
            f['pa_tiros_dominado'] = diff <= -4
    ...
    if right:
        ...
        if sc:
            ...
            og = sc[1] if ih else sc[0]
            f['ind_r_gano'] = tg > og
            f['ind_r_perdio'] = tg < og
            f['ind_r_goleo'] = tg >= 3
    
    # COMBINADOS para AH 0.5
    f['h_momentum'] = f.get('ph_gano', False) and f.get('pa_perdio', False)
    f['a_momentum'] = f.get('pa_gano', False) and f.get('ph_perdio', False)
    f['h_todo_bien'] = f.get('ph_gano', False) and f.get('h_fuerte', False) and f.get('h_mejor_rank', False)
    f['a_todo_bien'] = f.get('pa_gano', False) and f.get('a_fuerte', False) and f.get('a_mejor_rank', False)
    f['h_stats_dom'] = f.get('ph_domino', False) and f.get('ph_tiros_dom', False)
    f['a_stats_dom'] = f.get('pa_domino', False) and f.get('pa_tiros_dom', False)
    f['h_confirm'] = f.get('h2h_h_gano', False) and f.get('std_cov', False)
    f['a_confirm'] = f.get('h2h_a_gano', False) and f.get('gen_cov', False)
    
    return f


FEATURES = [
    'fav_local', 'fav_visita',
    'ou_bajo', 'ou_normal', 'ou_alto',
    'h_top5', 'a_top5', 'h_top10', 'a_top10', 'h_bottom', 'a_bottom',
    'h_mejor_rank', 'a_mejor_rank', 'ranks_cercanos',
    'h_fuerte', 'h_medio', 'h_debil', 'h_invicto',
    'a_fuerte', 'a_medio', 'a_debil', 'a_invicto',
    'h_anota_mucho', 'a_anota_mucho', 'h_recibe_poco', 'a_recibe_poco',
    'h_recibe_mucho', 'a_recibe_mucho', 'h_goleador', 'a_goleador',
    'h_defensivo', 'a_defensivo',
    'ph_gano', 'ph_empato', 'ph_perdio', 'ph_goleo', 'ph_recibio3', 'ph_porteria0',
    'ph_over', 'ph_under', 'ph_domino', 'ph_dominado', 'ph_tiros_dom',
    'pa_gano', 'pa_empato', 'pa_perdio', 'pa_goleo', 'pa_recibio3', 'pa_porteria0',
    'pa_over', 'pa_under', 'pa_domino', 'pa_dominado', 'pa_tiros_dom',
    'h2h_h_gano', 'h2h_a_gano', 'h2h_over', 'h2h_under',
    'std_cov', 'gen_cov', 'h2h_cov',
    'linea_subio', 'linea_bajo', 'linea_estable',
    'ind_l_gano', 'ind_l_perdio', 'ind_l_goleo',
    'ind_r_gano', 'ind_r_perdio', 'ind_r_goleo',
    'h_momentum', 'a_momentum', 'h_todo_bien', 'a_todo_bien',
    'h_stats_dom', 'a_stats_dom', 'h_confirm', 'a_confirm',
]


def main():
    print("=" * 60)
    print("ESPECIALISTA AH 0.5")
    print("Solo datos de handicap +0.5 y -0.5")
    print("=" * 60)
    ...
    print(f"\nTotal: {len(all_matches)} partidos AH 0.5")
    ...
    print(f"\nDistribucion AH:")
    print(f"  LOCAL cubre: {local_wins} ({local_wins*100/(local_wins+visita_wins):.1f}%)")
    print(f"  VISITA cubre: {visita_wins} ({visita_wins*100/(local_wins+visita_wins):.1f}%)")
    print(f"\nDistribucion O/U:")
    print(f"  OVER: {over_count} ({over_count*100/(over_count+under_count):.1f}%)")
    print(f"  UNDER: {under_count} ({under_count*100/(over_count+under_count):.1f}%)")
    ...
    for _ in range(3000):
        ...
        patterns_ah.append({'conds': [(c, True) for c in conds], 'pick': pick_ah, 'hits': 0, 'miss': 0})
        patterns_ou.append({'conds': [(c, True) for c in conds], 'pick': pick_ou, 'hits': 0, 'miss': 0})
    
    results_ah = []
    results_ou = []
    
    print(f"\nBuscando patrones AH 0.5 en {GENERATIONS} generaciones...")
    ...
    for gen in range(GENERATIONS):
        ...
        for p in patterns_ah:
            ...
            if total >= MIN_SAMPLES:
                ...
                if acc >= MIN_ACCURACY:
                    ...
                    if is_new and len(results_ah) < 50:
                        results_ah.append({
                            'pick': p['pick'],
                            'acc': round(acc, 1),
                            'hits': p['hits'],
                        ...
                        print(f"{stars} AH {p['pick']} {acc:.1f}% ({p['hits']}/{total}) <- {cond_names}")
        ...
        for p in patterns_ou:
            ...
            if total >= MIN_SAMPLES:
                ...
                if acc >= MIN_ACCURACY:
                    ...
                    if is_new and len(results_ou) < 50:
                        results_ou.append({
                            'pick': p['pick'],
                            'acc': round(acc, 1),
                            'hits': p['hits'],
                        ...
                        stars = '***' if acc >= 80 else '**' if acc >= 75 else '*'
                        cond_names = [c[0] for c in p['conds']]
                        print(f"{stars} OU {p['pick']} {acc:.1f}% ({p['hits']}/{total}) <- {cond_names}")
        ...
        for patterns in [patterns_ah, patterns_ou]:
            ...
            survivors = good[:500] if len(good) >= 500 else good
            
            new_patterns = list(survivors)
            while len(new_patterns) < 3000:
                if survivors:
                    ...
                    n = random.randint(2, 4)
                    conds = random.sample(FEATURES, min(n, len(FEATURES)))
                    pick = random.choice(['LOCAL', 'VISITA'] if patterns == patterns_ah else ['OVER', 'UNDER'])
            ...
            if patterns == patterns_ah:
                patterns_ah = new_patterns[:3000]
            else:
                patterns_ou = new_patterns[:3000]
    ...
    output = {
        ...
        'version': 'ah05-specialist-1.0',
        'matches': len(all_matches),
        'ah_patterns': sorted(results_ah, key=lambda x: -x['acc']),
        'ou_patterns': sorted(results_ou, key=lambda x: -x['acc'])
    }
    
    path = RESULTS_DIR / 'ah05_patterns.json'
    with open(path, 'w', encoding='utf-8') as f:
        json.dump(output, f, indent=2, ensure_ascii=False)
    
    print(f"\n{'='*60}")
    print(f"RESUMEN AH 0.5: AH={len(results_ah)} | OU={len(results_ou)}")
    
    if results_ah:
        print("\nTOP 10 AH:")
        for i, p in enumerate(sorted(results_ah, key=lambda x: -x['acc'])[:10], 1):
            cond_names = [c[0] for c in p['conds']]
            print(f"  {i}. {p['pick']} {p['acc']}% ({p['hits']}/{p['total']}) <- {cond_names}")
    
    if results_ou:
        print("\nTOP 10 OU:")
        for i, p in enumerate(sorted(results_ou, key=lambda x: -x['acc'])[:10], 1):
            cond_names = [c[0] for c in p['conds']]
            print(f"  {i}. {p['pick']} {p['acc']}% ({p['hits']}/{p['total']}) <- {cond_names}")
