...
def get_outcomes(home_g, away_g, ah, is_home_fav):
    ...
    # For AH 0, is_home_fav is ambiguous but let's assume home is the one we check
    if ah == 0:
        diff = home_g - away_g
        if diff > 0: f, u = "WIN", "LOSS"
...
def get_match_features(match):
    ...
    # For AH 0, we'll treat Home as the "fav" for feature extraction purposes
    ...
    gah_str = f_p_m.get("handicap_line_raw")
    gah = float(gah_str) if gah_str and gah_str != "-" else 0
    ...
    ugah_str = u_p_m.get("handicap_line_raw")
    ugah = float(ugah_str) if ugah_str and ugah_str != "-" else 0
    ...
    sah_str = h_s.get("ah1")
    sah = float(sah_str) if sah_str and sah_str != "-" else 0
...
def main():
    ...
    for k, s in megastats.items():
        if s["total"] < 15: continue # Threshold to reach 200
    ...
    # Sort prioritizing Sample Size, then ROI
    ...
    print(f"Patrones encontrados: {len(final_list)}")
