#!/usr/bin/env python3
"""
Genera un informe Top-N de partidos "No Favorito + Over" para hoy
...
class EncodedMatch:
    ...
    underdog_win: Optional[bool]
    underdog_over_hit: Optional[bool]
    features: Dict[str, Any]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Top de no-favoritos+over con patrones del explorador"
    )
    parser.add_argument(
        "--project-root",
        default=".",
    ...
    parser.add_argument(
        "--output-md",
        default="report_no_favorito_over_hoy.md",
        help="Ruta de salida markdown.",
    )
    parser.add_argument(
        "--output-json",
        default="report_no_favorito_over_hoy.json",
...
def encode_match(match: Dict[str, Any], historical: bool) -> Optional[EncodedMatch]:
    ...
    if parsed_score:
        ...
        if fav_side == "HOME":
            ...
            underdog_win = hg > ag
        underdog_over_hit = bool(underdog_win and over_hit) if over_hit is not None else None
    else:
        hg = ag = None
        over_hit = None
        underdog_win = None
        underdog_over_hit = None
    ...
    return EncodedMatch(
        ...
        underdog_over_hit=underdog_over_hit,
        features=features,
    )
    ...
    for cand in today_candidates:
        ...
        for hist in historical:
            if hist.match_id == cand.match_id:
                continue
            if hist.underdog_over_hit is None:
                continue
        ...
        w_hits = sum(sim for hist, sim in sampled if hist.underdog_over_hit)
        if w_total <= 0:
            continue
        ...
        pick_side = "VISITANTE" if cand.fav_side == "HOME" else "LOCAL"
        ...
        ranked.append(
            {
                ...
                "non_favorite_side": pick_side,
                "ah": cand.ah,
                "ou_line": cand.ou,
                "probability_no_fav_over": round(p_hat * 100.0, 2),
                "support": support,
...
def render_markdown(report: Dict[str, Any], target_day: date) -> str:
    top = report.get("top", [])
    lines = [
        f"# Informe No Favorito + Over ({target_day.isoformat()})",
        "",
        "Metodo:",
    ...
    lines.extend(
        [
            ...
            "",
            "| # | Partido | No Favorito | AH | OU | Prob NoFav+Over | Soporte |",
            "|---|---|---|---:|---:|---:|---:|",
        ]
    )
    ...
    for idx, row in enumerate(top, 1):
        lines.append(
            f"| {idx} | {row['home']} vs {row['away']} | {row['non_favorite_side']} | "
            f"{row['ah']} | {row['ou_line']} | {row['probability_no_fav_over']}% | {row['support']} |"
        )
    ...
    for idx, row in enumerate(top, 1):
        lines.append(
            ...
            f"{row['reason']} Prob. estimada no-favorito+over: {row['probability_no_fav_over']}% "
...
def load_project_data(
    ...
    for m in historical_rows:
        ...
        if enc and enc.underdog_over_hit is not None:
