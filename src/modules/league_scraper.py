...
def extract_ids_by_params(season, league_id, ah_filter=None):
    ...
    if ah_filter:
        try:
            # Robust parsing: handle commas as decimals, parentheses, etc.
            # 1. Normalize commas to dots for decimal standard
            normalized_filter = ah_filter.replace(',', '.')
            
            # 2. Extract numbers using regex (matches 0.5, -0.5, 1, -1, .5)
            # Pattern: optional sign, optional digits, optional dot, digits
            matches = re.findall(r'[-+]?\d*\.?\d+', normalized_filter)
            
            for m in matches:
                # Filter out pure symbols like "." or "-" if regex matches them standalone
                if m in ['.', '-', '+', '']: continue
                try:
